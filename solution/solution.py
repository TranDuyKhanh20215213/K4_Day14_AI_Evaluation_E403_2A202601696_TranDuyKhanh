...
class LLMJudge:
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for i, entry in enumerate(scores_batch):
            ...
            if i == 0:
                first_scores.extend(values)
            else:
                rest_scores.extend(values)
...
class FailureAnalyzer:
    ...
    def generate_improvement_suggestions(
        ...
        if categories.get("irrelevant"):
            suggestions.append(
                "Improve prompt clarity and intent routing so answers stay on-topic"
            )
        if categories.get("incomplete"):
            suggestions.append(
                "Increase chunk size or add few-shot examples showing complete answers "
                "to improve completeness"
            )
        if categories.get("off_topic"):
            suggestions.append(
                "Review intent detection and routing to prevent off-topic answers"
            )
        if categories.get("refusal"):
            suggestions.append(
                "Loosen overly strict guardrails that trigger unnecessary refusals"
            )

        generic_fallbacks = [
            "Expand the golden dataset to cover more edge cases and re-run the benchmark",
            "Add regression tests to catch metric drops before deployment",
            "Review the retrieval and generation pipeline end-to-end for the lowest-scoring cases",
        ]
